...
* `regions`: una matriz de valores basada en el `regionType`
* `status`: el estado de la regla actual, activa (`ACTIVE`) o inactiva (`INACTIVE`)

si `regionType` = `CONTINENT`,
```
  regions = [
                  Africa,
                  Asia,
...
si `regionType` = `COUNTRY_OR_REGION`,
```
  regions = [
                    Andorra,
