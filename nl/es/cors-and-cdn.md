...
En el caso de una comunicación CORS más compleja entre el navegador y un origen de CORS distinto de la página web de la solicitud, se necesita una [solicitud previa](https://www.w3.org/TR/cors/#cross-origin-request-with-preflight-0) antes de un acceso al recurso real. En algunas situaciones se pueden requerir solicitudes CORS previas, como métodos HTTP que no son métodos `GET` o `POST`,
