...
請參閱[疑難排解](/docs/infrastructure/CDN/troubleshooting.html#troubleshooting)或[取得協助及支援](/docs/infrastructure/CDN/getting-help.html#gettinghelp)頁面，或在[客戶入口網站 ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://control.softlayer.com/) 中開立問題單。
...
如果您是帳戶的次要「使用者」之一，則帳戶的「管理員使用者」必須給您 `Add/Upgrade Services` 許可權，才能在此頁面上顯示或啟用「建立」按鈕。從 IBM Cloud 主控台頁面，帳戶的「管理員使用者」應遵循下列步驟以編輯您的許可權：
 * 選取**管理**標籤
 * 選取**存取 (IAM)**
 * 按一下左窗格中的**使用者**標籤
 * 按一下想要的**使用者**
