...
单击需要验证的 CDN 的名称。此时将打开“概述”页面，在其中可以查看 CDN 的总体状态。在页面顶部会有一个警报，提醒您需要进行域验证。选择**查看域验证**按钮以打开一个窗口，其中显示完成验证过程所需的质询信息。
...
域验证质询用于证明您是域的所有者。下面是可以处理域验证质询的三种方法。

**注**：如果您未在 48 小时内响应域验证请求，那么请求会到期，您需要重新开始订购过程。

### CNAME

如果在订购 CDN 之前，已将 CNAME 记录添加到 DNS 提供者，那么不需要执行其他任何操作。域验证将由 IBM Cloud、Akamai 和 CA 自动处理。验证可能需要 2 到 4 小时。

  * 如果您尚未使用 DNS 提供者来配置 CNAME，那么此时需要进行配置。大部分 DNS 提供者都能够在设置和更改 CNAME 方面给予您指导。

   ![域验证 - CNAME](images/domain-validation-cname.png)

**注**：**仅当** CDN **不**提供实时流量时，才建议使用此方法。如果域正在提供实时流量，那么建议您使用“标准”方法来验证域。

---
### 标准

如果选择“标准”方法来进行域验证，那么“域验证”窗口将显示**质询 URL** 和**质询响应**。要完成域验证过程，必须将提供的**质询响应**添加到源服务器。这允许 CA 使用在**质询 URL** 中指定的 URL 从源服务器中检索**质询响应**。正确配置源服务器后，域验证可能需要 2 到 4 小时。

   ![域验证质询 - 标准](images/domain-validation-standard.png)

要通过“标准”方法成功完成域验证，需要以特定方式配置源服务器。下面概述了 Apache 和 Nginx 服务器的示例过程。
...
要通过“重定向”方法成功完成域验证，您可能需要以特定方式配置 Web 服务器。下面概述了 Apache 和 Nginx 服务器的示例过程。
...
#### Nginx 重定向配置
  ...
  * **步骤 5：**在 DNS 中创建 CDN 域和源服务器的 IP 之间的一个 A 记录。

#### 验证是否正在执行重定向

完成以下步骤可仅将特定质询 URL 的流量重定向到重定向 URL。可以通过 `curl` 或浏览器来验证重定向是否正常工作。
