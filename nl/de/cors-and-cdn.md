...
Ein Hinweis: **Ein Ursprung in Bezug auf CORS muss nicht mit einem CDN-Ursprung identisch sein**. Ein Ursprung in CORS wird durch ein URI-Schema, eine Domäne und eine beliebige mögliche Portnummer definiert. Beispielsweise ist https://www.example.com:1443 ein anderer Ursprung als http://www.example.com. Daher kann ein CDN aus der Perspektive eines Browsers auch als CORS-Ursprung betrachtet werden. 
...
[Einfache Anforderungen](https://www.w3.org/TR/cors/#simple-cross-origin-request) über CORS sind entweder `GET`- oder `POST`-Anforderungen von der Webseite eines einzigen Ursprungs, die versuchen, Zugriff auf die URL eines anderen Ursprungs für Ressourcen zu erhalten. 

Bei solchen Anforderungen setzt der Browser automatisch CORS-Anforderungsheader. In erster Linie setzt der Browser den Ursprung der Webseite, die die Anforderung sendet, im HTTP-Header `Origin` automatisch. Die CORS-Anforderung kann auch eine Gruppe von [bestimmten Standard-HTTP-Headern](https://www.w3.org/TR/cors/#simple-header) enthalten, während sie ihren Status als einfache CORS-Anforderung aus der Perspektive des Browsers beibehält. 
...
Im Allgemeinen sollte der Browser gemäß der [W3C-Spezifikation in Bezug auf diesen Platzhalterwert](https://www.w3.org/TR/cors/#security) Inhalte unbegrenzt laden, wenn die CORS-Antwortheader des Servers den Code `Access-Control-Allow-Origin: *` enthalten. Jedoch wird `Access-Control-Allow-Origin: *` nicht von allen Browsern unterstützt. 
